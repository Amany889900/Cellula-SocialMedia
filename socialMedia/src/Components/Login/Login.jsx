...
<form className="max-w-md mx-auto mt-15" onSubmit={handleSubmit(async(data) => {
  ...
  {loginInputs.map((input,idx)=>(
    <InputField key={idx} type={input.type} name={input.name} id={input.id} placeholder={input.placeholder} label={input.label} {...register(input.register)}/>
  ))}
 ...
 <p className="mt-5 text-[#b3b3b3] text-center mb-40">Don’t have account? Let’s <Link className="text-[#34D15C] underline">Sign up</Link> </p>
