...
//   const userId = JSON.parse(localStorage.getItem("userId"))
  ...
  useEffect(() => {
    dispatch(getFollowers(follower_id));
  }, [followers.length]);

    useEffect(() => {
    dispatch(getFollowing(follower_id));
  }, [following.length]);
   ...
   <>
    ...
    <div  className="w-[80%] mx-auto mt-15 ">
        <div className="flex flex-col items-center">
         ...
         <div>
            <button onClick={()=>{navigate(`/chat/${follower_id}`)}} className="cursor-pointer focus:ring-4 border-2 border-[#EEEEEE] focus:outline-none focus:ring-green-300 font-medium rounded-full text-sm w-full sm:w-auto px-5 py-4 text-center mt-10"><i className="fa-solid fa-message"></i> Message</button>
           {/* <button onClick={()=>{navigate("/editProfile")}} className="cursor-pointer focus:ring-4 border-2 border-[#EEEEEE] focus:outline-none focus:ring-green-300 font-medium rounded-full text-sm w-full sm:w-auto px-5 py-4 text-center mt-10">Follow</button> */}
         </div>
